    ...
    function partition(Participant[] storage arr, int256 left, int256 right) internal returns (int256) {
        ...
        for (int256 j = left; j < right; j++) {
            if (arr[uint256(j)].points > pivot) {
                i++;
                // Separate the assignments to avoid storage overwrite
                Participant memory temp1 = arr[uint256(i)];
        ...
        Participant memory swapTemp = arr[uint256(i + 1)];
        arr[uint256(i + 1)] = arr[uint256(right)];
        arr[uint256(right)] = swapTemp;
